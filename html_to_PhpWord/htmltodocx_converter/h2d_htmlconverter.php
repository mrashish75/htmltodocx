        ...
        switch ($element->tag) {
            ...
            case 'table':
                if (in_array('table', $allowed_children)) {
                    ...
                    //table cells margins
                    $state['current_style']['_cellMarginLeft'] = 80;
                    $state['current_style']['_cellMarginRight'] = 80;
                    $state['current_style']['_cellMarginTop'] = 20;
                    $state['current_style']['_cellMarginBottom'] = 20;
            ...
            case 'img':
                ...
                try {
                    if ($element->height && $element->width) {
                        ...
                        $state['current_style']['height'] = 250;
                        $state['current_style']['width'] = 250;
                    }

                    if (strpos($element->src, $state['base_root']) === 0) {
                        // The image source is a full url, but nevertheless it is on this
                        // server.
                        $element_src = trim(substr($element->src, strlen($state['base_root'])));
                    } else {
                        $element_src = trim($element->src);
                    }
                    // $element_src=str_replace(' ', '%20', $element_src);
                    if (strpos($element_src, 'http://') === 0 || strpos($element_src, 'https://') === 0) {
                        ...
                        if (@getimagesize($element_src)) {
                            $fileExtension = pathinfo($element_src, PATHINFO_EXTENSION);
                            $tmpfile = @file_get_contents($element_src);
                            ...
                            $fullfilename = $tmpDir . "/" . gen_uuid() . "." . $fileExtension;
                            @file_put_contents($fullfilename, $tmpfile);
                            $src = $fullfilename;
                        } else {
                            $src = $element_src;
                        }
                    ...
                    if (!is_file($src)) {
                        if ($newurl = createImageFromBase64ToLocalpath($element_src)) {
                            $src = $newurl;
                        } else {
                            $src = htmltodocx_doc_root() . $state['base_path'] . 'img/performance/placeholder.png';
                        }
                    }
...
/**
 * @description this is function only use for create temp image if image src is data:base64
 * @param $base64_string
 * @return bool
 */
function createImageFromBase64ToLocalpath($base64_string){
    if(@getimagesize($base64_string)){
        $imagestr = strstr($base64_string, ',');
        if($imagestr) {
